...
| `sort_by_label`      | `string`  | `ascending`  | Sort the tasks by their label. (This works best when you're using single labels per item). If it is undefined, no sorting occurs. If it is set to `'ascending'` (or anything else), it sorts alphabetically by label. If it is set to `'descending'`) it will sort in reverse alphabetic order. |
